...
export async function POST(request: Request) {
  ...
  if (!membership?.organization_id) return NextResponse.json({ error: "No workspace found." }, { status: 403 });
  ...
  const { error } = await admin.from("integration_connections").upsert({
    ...
    metadata: { auth_provider: "google", google_refresh_token_encrypted: encrypted, google_refresh_token_saved_at: new Date().toISOString() },
    last_error: null,
