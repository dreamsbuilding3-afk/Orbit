...
export default function ArkRecommendations(){
 ...
 async function approve(item:Recommendation){
  ...
  const {data:{user}}=await supabase.auth.getUser(); if(!user){setMessage("Connecte-toi à WineTime.");setBusy(null);return;}
  const {data:{session}}=await supabase.auth.getSession(); const providerToken=session?.provider_token;
  if(!providerToken){setMessage("Reconnecte Gmail pour permettre à WineTime d'exécuter cette action.");setBusy(null);return;}
 ...
 async function execute(item:Recommendation){
  ...
  try{
   ...
   setItems(current=>current.map(x=>x.id===item.id?{...x,status:"executed"}:x)); setMessage("✅ WineTime executed the Gmail action.");
  }catch(error){setMessage(error instanceof Error?error.message:"Impossible d'exécuter l'action.");}finally{setBusy(null);}
 }
 return <main className="ark-shell"><header className="ark-topbar"><div className="ark-brand"><Link href="/ark">WineTime</Link><span>/</span><strong>Recommendations</strong></div><nav className="ark-nav"><Link href="/ark">Brief</Link><Link href="/ark/opportunities">Opportunities</Link><Link className="active" href="/ark/recommendations">Recommendations</Link><Link href="/ark/memory">Memory</Link></nav></header><div className="ark-page"><section className="ark-hero"><div><p className="eyebrow">ARK ACTIONS</p><h1>Decide what<br/>happens next.</h1><p>ARK prepares the decision. WineTime executes it. Sensitive actions remain behind your permissions.</p></div><div className="ark-state"><span/>Human control active</div></section>{message&&<div className="notice">{message}</div>}<section className="brief-section"><div className="section-head"><div><p className="eyebrow">PENDING DECISIONS</p><h2>Recommendations waiting for you</h2></div></div><div className="memory-list">{items.length?items.map(item=> <article className="rec-row" key={item.id}><div className="rec-main"><span className="memory-cat">{item.status}</span><h3>{item.title}</h3><p>{item.explanation}</p><small>{item.confidence==null?"Confidence —":`Confidence ${Math.round(item.confidence*100)}%`}</small></div><div className="rec-actions">{item.status==="pending"?<button className="brief-button" disabled={busy===item.id} onClick={()=>approve(item)}>{busy===item.id?"Checking…":"Approve"}</button>:item.status==="approved"?<button className="brief-button" disabled={busy===item.id} onClick={()=>execute(item)}>{busy===item.id?"Executing…":"Execute"}</button>:<span className="run-status approved">{item.status}</span>}</div></article>):<div className="empty-state"><strong>ARK has no pending recommendations.</strong><span>Recommendations will appear here when business signals produce a defensible next action.</span></div>}</div></section></div><style jsx>{`.notice{margin-bottom:24px;padding:14px 16px;border:1px solid rgba(0,0,0,.08);border-radius:12px;background:rgba(255,255,255,.8);font-size:11px;color:#555;line-height:1.5}.rec-row{display:flex;justify-content:space-between;align-items:flex-start;gap:32px;padding:28px 26px;border-bottom:1px solid rgba(0,0,0,.05);background:rgba(255,255,255,.36);transition:.18s ease}.rec-row:hover{background:rgba(255,255,255,.7)}.rec-row:last-child{border-bottom:0}.rec-main{min-width:0;max-width:820px}.rec-main h3{margin:9px 0 8px;font-size:17px;line-height:1.3;letter-spacing:-.025em;font-weight:600}.rec-main p{margin:0;color:#777;font-size:13px;line-height:1.7;max-width:720px}.rec-main small{display:block;margin-top:13px;color:#aaa;font-size:10px;line-height:1.5}.rec-actions{min-width:112px;text-align:right;padding-top:3px}.rec-actions .brief-button{min-width:96px;justify-content:center}@media(max-width:700px){.rec-row{display:block;padding:22px 18px}.rec-actions{text-align:left;margin-top:18px}.rec-main h3{font-size:16px}.rec-main p{font-size:12px}}`}</style></main>;
}
